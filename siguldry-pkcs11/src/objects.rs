...
use cryptoki_sys::{
    CK_ATTRIBUTE, CK_ATTRIBUTE_TYPE, CK_BBOOL, CK_FALSE, CK_KEY_TYPE, CK_OBJECT_CLASS,
    CK_OBJECT_HANDLE, CK_RV, CK_TRUE, CK_ULONG, CK_UNAVAILABLE_INFORMATION, CKA_ALLOWED_MECHANISMS,
    CKA_ALWAYS_AUTHENTICATE, CKA_ALWAYS_SENSITIVE, CKA_CLASS, CKA_COPYABLE, CKA_DECRYPT,
    CKA_EC_PARAMS, CKA_EC_POINT, CKA_ENCRYPT, CKA_EXTRACTABLE, CKA_ID, CKA_KEY_TYPE, CKA_LABEL,
    CKA_MODULUS, CKA_MODULUS_BITS, CKA_NEVER_EXTRACTABLE, CKA_PUBLIC_EXPONENT, CKA_PUBLIC_KEY_INFO,
    CKA_SENSITIVE, CKA_SIGN, CKA_SIGN_RECOVER, CKA_TOKEN, CKA_TRUSTED, CKA_UNWRAP, CKA_VERIFY,
    CKA_WRAP, CKK_EC, CKK_RSA, CKO_CERTIFICATE, CKO_PRIVATE_KEY, CKO_PUBLIC_KEY,
...
impl Object {
    ...
    pub(crate) fn from_key(key: Key) -> HashMap<CK_OBJECT_HANDLE, Object> {
        let mut objects = HashMap::new();
        if let Some(o) = objects.insert(
            Self::PRIVATE_KEY_HANDLE,
            Self {
                attributes: Attribute::from_private_key(&key),
            },
        ) {
            panic!("evicted object {o:?}")
        }
        if let Some(o) = objects.insert(
...
impl From<*mut CK_ATTRIBUTE> for Attribute {
    fn from(value: *mut CK_ATTRIBUTE) -> Self {
        if value.is_null() {
            panic!("attribute pointer is null");
        }
...
impl Attribute {
    fn from_private_key(key: &Key) -> HashMap<u64, Attribute> {
    ...
    fn from_public_key(key: &Key) -> HashMap<u64, Attribute> {
        let mut attrs = Self::common_attributes(key);
    ...
    fn from_certificate(key: &Key, _cert: &Certificate) -> HashMap<u64, Attribute> {
        let mut attrs = Self::common_attributes(key);
    ...
    fn common_attributes(key: &Key) -> HashMap<u64, Attribute> {
        ...
        let key_type: CK_KEY_TYPE = match key.key_algorithm {
            siguldry::protocol::KeyAlgorithm::Rsa4K | siguldry::protocol::KeyAlgorithm::Rsa2K => {
                CKK_RSA
            }
            siguldry::protocol::KeyAlgorithm::P256 => CKK_EC,
            _ => todo!(),
        };
        let key_bits: CK_ULONG = match key.key_algorithm {
            siguldry::protocol::KeyAlgorithm::Rsa4K => 4096_u64,
            siguldry::protocol::KeyAlgorithm::Rsa2K => 2048_u64,
            siguldry::protocol::KeyAlgorithm::P256 => 256_u64,
            _ => todo!(),
        };
        attrs.insert(
            CKA_KEY_TYPE,
            Attribute {
                ...
                value: key_type.to_ne_bytes().to_vec(),
            },
        );
        attrs.insert(
            CKA_TOKEN,
            ...
            KeyAlgorithm::Rsa4K | KeyAlgorithm::Rsa2K => {
                    ...
                    openssl::rsa::Rsa::public_key_from_pem(key.public_key.as_bytes())
                {
                    ...
                    attrs.insert(
                        ...
                        Attribute {
                            attribute_type: CKA_MODULUS_BITS,
                            value: key_bits.to_ne_bytes().to_vec(),
                        },
                    );
                    attrs.insert(
                        CKA_PUBLIC_KEY_INFO,
                        Attribute {
                            ...
                            value: pubkey.public_key_to_der().unwrap(),
                        },
                    );
                }
                ...
                if let Ok(cert) = sequoia_openpgp::Cert::from_bytes(key.public_key.as_bytes()) {
                    ...
                    match cert.primary_key().key().mpis() {
                        sequoia_openpgp::crypto::mpi::PublicKey::ECDSA { curve: _, q } => {
                            ...
                            let der_point = asn1::write_single(&octet_point).unwrap();
                            attrs.insert(
                                CKA_EC_POINT,
                                Attribute {
                            ...
                            let pubkey_der = asn1::write_single(&SubjectPublicKeyInfo {
                                ...
                                subject_public_key: asn1::BitString::new(q.value(), 0).unwrap(),
                            })
                            .unwrap();
                            attrs.insert(
                                CKA_PUBLIC_KEY_INFO,
                        ...
                        unsupported => {
                            tracing::error!(key_algorithm=?unsupported.algo(), "Unsupported key algorithm");
                        }
                    };
                } else if let Ok(pubkey) =
