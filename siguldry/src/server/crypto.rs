...
use crate::server::{config::Pkcs11Binding, db::KeyType};
...
pub fn create_encrypted_file_key(
    config: &super::Config,
    ...
    let key = match key_type {
        KeyType::Rsa4K => PKey::from_rsa(Rsa::generate(4096)?)?,
        KeyType::Ed25519 => PKey::generate_ed25519()?,
        KeyType::P256 => PKey::from_ec_key(EcKey::generate(
    ...
    let key_dir = config.keypair_dir(&handle)?;
...
enum BoundPassword {
    ...
    ///
    /// Secrets of this variant have been encrypted using OpenSSL's pkcs7 interface and the
    ...
    Pkcs11WithPkcs7 {
        ...
        /// The string contains a PEM-encoded PKCS7 structure.
        password: String,
    },
}
...
pub(crate) async fn decrypt_key_password(
    bindings: &[Pkcs11Binding],
    user_password: Password,
    data: &[u8],
...
pub(crate) fn encrypt_key_password(
    bindings: &[Pkcs11Binding],
    user_password: Password,
...
fn binding_encrypt(binding: &Pkcs11Binding, data: &[u8]) -> anyhow::Result<BoundPassword> {
    ...
    let key_fingerprint = certificate
        .digest(MessageDigest::sha256())?
        .iter()
        .map(|byte| format!("{byte:02x}"))
        .collect::<String>();
    Ok(BoundPassword::Pkcs11WithPkcs7 {
        key_fingerprint,
...
async fn binding_decrypt(binding: Pkcs11Binding, data: Vec<u8>) -> anyhow::Result<Vec<u8>> {
    let output = tokio::task::spawn_blocking(move || {
        ...
        let mut stdin = child
            .stdin
            .take()
            .ok_or_else(|| anyhow::anyhow!("openssl-smime command missing stdin"))?;
    ...
    if !output.status.success() {
        return Err(anyhow::anyhow!(
            "Failed to decrypt data via PKCS#11 using openssl-smime (exited {:?}): {stderr}",
            output.status.code()
        ));
    }
...
mod tests {
    ...
    fn setup_softhsm() -> anyhow::Result<SoftHsm> {
        ...
        let uri = uri_line.split_whitespace().last().unwrap().trim();
