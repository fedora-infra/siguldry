...
use sequoia_openpgp::{crypto::Password, parse::Parse, serialize::MarshalInto};
use siguldry::{
...
impl SigulKey {
    ...
    fn certificates_pem(&self) -> anyhow::Result<Vec<String>> {
            ...
            .filter_map(|dir_entry| {
                if let Ok(dir_entry) = dir_entry {
                        ...
                        && name.starts_with(&cert_format)
                    {
                        ...
                        std::fs::read_to_string(dir_entry.path()).ok()
                    } else {
                        None
                    }
                } else {
    ...
    async fn import_keypair(
        ...
        let key_material = sigul_key_password.map(|sigul_key_password| {
            let privkey = openssl::pkey::PKey::private_key_from_pem_passphrase(
                privkey_pem.as_bytes(),
                sigul_key_password,
            )?;
            tracing::debug!("Successfully decrypted private key");
            privkey
                .private_key_to_pem_pkcs8_passphrase(Cipher::aes_256_cbc(), sigul_key_password)
        ...
        for cert in self.certificates_pem()? {
            db::PublicKeyMaterial::create(conn, &key, db::PublicKeyMaterialType::X509, cert)
                .await?;
        }
    ...
    async fn import_gnupg(
        ...
        let cert = sequoia_openpgp::Cert::from_bytes(&secret_output.stdout)
        ...
        cert.keys()
            ...
            .key()
            .clone()
            .decrypt_secret(&sigul_key_password)?;
        ...
        let key_algorithm = match cert.primary_key().key().pk_algo() {
            ...
            other => {
                tracing::warn!(sigul_key = self.name, algorithm = ?other, "GPG key uses unsupported algorithm");
                return Err(anyhow::anyhow!("Unsupported key algorithm {:?}", other));
            }
        };

        // Be certain we're not about to write a secret key that's unencrypted
        if cert
            .keys()
            .secret()
